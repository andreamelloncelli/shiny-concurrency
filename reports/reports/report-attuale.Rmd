---
title: "Report Progetto Shiny Concurrency"
...
* 1 AWS server con Shiny Proxy (si è reso necessario un 4 core, 16 Gb di RAM, vedi nella sezione tests)
...
### Primo test su Shiny proxy (NEW)

Il test è il medesimo (Primo test) ed è stato effettuato sulla medesima applicazione. Tuttavia è stato ri-registrato e nei report il primo click è stato perso per qualche ragione, ma il test è stato il medesimo: caricamento pagina piu' due cambi data.

La modalità con cui è stato effettuato questo test è diversa. Dapprima sono state avviate tutte le istanze e poi sono stati lanciati i test come nel caso di Shiny Server Pro. Quindi al momento dei test il numero massimo di processi R (Shiny) era gia' attivo.
...
11	2017-12-05T23:29:50.279report_output_11_mysql_200usr_4thr_100-090-40_2min_net.Z	347	REQ	0
...
* Utenti concorrenti : 1
* durata minima del test: 2minuti
* Sever che attua il test: remota
...
* Utenti concorrenti : 1
* durata minima del test: 2minuti
* Sever che attua il test: in rete locale (AWS)
...
### Test Shiny Proxy (NEW)

Limitazioni riscontrate:
* il 2 core 8 Gb di ram non riusciva a tenere le 150 utenze (container docker) contemporaneamente attive. Ho installato il software su un hardware piu' performante: 4 core 16Gb.
* utilizzare N istanze diverse richiede N connessioni al db. In questo caso quindi ho fatto un test con N=150 connessioni massime, perché 200 non erano supportate dal db. Probabilmente è solo una questione di configurazioni: tra 150 e 200 non dovrebbe cambiare molto per il database. La banda usata deve essere la stessa del test con Shiny server Pro, è solo divisa su piu' connessioni.
* Limitazione della modalità di test: non è stato possibile simulare gli N browser in tempo reale e il test è stato diviso in 2 parti: l'*avvio delle singole utenze* (container docker) e poi *test di concorrenza*. Vedi sopra la descrizione del test. 

*1) Avvio delle singole utenze*:

Tempo necessario per fare partire 150 container: `1 minuto 31.589 secondi`.

*2) Test di concorrenza*

[test 70](./report__proxy_output_70_mysql_150usr_4thr_100-090-40_2min_net.html):

* Utenti concorrenti : 150
* durata minima del test: 2minuti
* Sever che attua il test: in rete locale (AWS)

**Osservazioni**

* Un click non viene rilevato: l'unico modo di risolvere il problema è ri-registrare il test. Ad ogni modo ogni utenza carica la pagina e fa 2 richieste al db (cambi data).
* I tempi di caricamento della pagina sono inferiori: questo è riconducibile al fatto che l'applicazione sia gia' attiva e/o al fatto che l'hardware sia piu' potente.
